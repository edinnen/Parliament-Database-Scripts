...
#Find contractions and return the expanded
...
### Can't remember why I thought needed this in V1
def tokenizer(match):
    ...
    while len(chunks):
      ...
      if len(buf) == 0:
        continue
      if re.match(r'^[A-ZÀÂÇÉÈÊËÎÏÔÛÙÜŸÑÆŒ]$', buf) and len(chunks):
        buf += chunks.popleft()
      result.append(buf)
...
def doTheClean(theFile):
    ...
    #Match everything below/including <!-- Publication Content --> and store it in out
    tmp = re.search(".+?(<!-- Publication Content\. -->)(.|\n)*", allTheHTML)
    out = tmp.group(0)
    ...
    out = re.sub("<!-- Publication Content\. -->", "", out) #Remove <!-- Publication Content -->
    out = re.sub(".+?(<!-- Publication Sidebar\. -->)(.|\n)*", "", out)
    ...
    #out = re.sub("(^|[^\n])\n(?!\n)", " ", out) #Not sure what this one does but it breaks things. Also randomly matches the last char of a line
    #out = re.sub("[^a-z^(àâçéèêëîïôûùüÿñæœ)A-Z^(ÀÂÇÉÈÊËÎÏÔÛÙÜŸÑÆŒ)\s]", "", out) #Anything that isn't whitespace or letters
    out = re.sub("\t", "", out)
    out = re.sub("[A-ZÀÂÇÉÈÊËÎÏÔÛÙÜŸÑÆŒ]{2,}", "", out)
    out = re.sub("\w*html", "", out)
    out = re.sub("0+", "", out)
    out = re.sub("\n{2,}", "\n", out)
    out = re.sub("\|", "", out)
    out = ' '.join(out.split())

    # out = re.sub("<!((.|\n)*)(The House met at)", "", allTheHTML) #Remove everything to 'the house met at'
    # out = re.sub(".+?(<!-- Publication Content\. -->)(.|\n)*", "", out)
    # out = re.sub("<.*?>", "", out) #Anything between <...>
    # out = re.sub("\**", "", out) #Any *
    # out = re.sub("\[.*?\]", "", out) #Any [...]
    # out = re.sub("\(.*?\)", "", out) #Any (...)
    # out = re.sub("[1-9]+", numToWord, out) #Any change any int to words
    # out = re.sub("point zero", "", out) #Any 'point zero'
    # out = re.sub("[!\"#$%&'\(\)\.\*\+,-\/:;—<=>\?@\\\^_`\{\}~]", "", out) #Any special char
    # out = re.sub("(^|[^\n])\n(?!\n)", " ", out) #
    # out = re.sub("\t", "", out)
    ...
    # out = re.sub("\w*html", "", out)
    # out = re.sub("0+", "", out)
    # out = re.sub("[a-z]+[A-Z]{1}[a-z]*", tokenizer, out) #Matches mixed case strings (i.e. C[hattersMr]) and passes to the tokenizer function
    # out = re.sub("\n{2,}", "\n", out)
    # out = re.sub("\|", "", out)
